...
# Option A (legacy): Create an anonymous account
# NOTE: Sesame may reject anonymous accounts (403) as policies evolve.
signup_response = client.create_anonymous_account()
print(f"Anonymous ID Token: {signup_response.id_token}")

# Option B (recommended): Sign in with email/password (refreshable)
signin_response = client.sign_in_with_email_password("you@example.com", "your-password")
print(f"Signed-in ID Token: {signin_response.id_token}")
...
### Environment-based auth (recommended for automation)

`TokenManager` will prefer these in order:

- `SESAME_ID_TOKEN`: Use a manually-provided ID token as-is (quick testing).
- `SESAME_EMAIL` + `SESAME_PASSWORD`: Sign in and persist refreshable tokens.

Example (PowerShell):

```powershell
$env:SESAME_EMAIL="you@example.com"
$env:SESAME_PASSWORD="your-password"
python examples/voice_chat.py --token-file token.json
```
