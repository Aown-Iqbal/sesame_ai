...
class SesameAI:
    ...
    def sign_in_with_email_password(self, email, password):
        """
        Sign in with email/password (Firebase Auth: accounts:signInWithPassword).

        This returns an ID token + refresh token that can be refreshed via
        ...
        response_json = self._make_auth_request("signInWithPassword", payload)
        return SignupResponse(response_json)
