...
class TokenManager:
    ...
    def get_valid_token(self, force_new=False):
        ...
        """
        # Highest priority: explicit ID token (manual paste / env injection).
        # This is useful when Sesame changes auth flows and you need to validate
        # WebSocket behavior quickly with a known-good token.
        env_id_token = os.environ.get("SESAME_ID_TOKEN")
        if env_id_token:
            logger.info("Using ID token from SESAME_ID_TOKEN")
            return env_id_token

        # Next priority: sign in with email/password (refreshable).
        env_email = os.environ.get("SESAME_EMAIL")
        env_password = os.environ.get("SESAME_PASSWORD")
        if env_email and env_password:
            logger.info("Signing in using SESAME_EMAIL/SESAME_PASSWORD")
            signin_response = self.api_client.sign_in_with_email_password(env_email, env_password)
            self.tokens = {
                'id_token': signin_response.id_token,
                'refresh_token': signin_response.refresh_token,
                'user_id': signin_response.local_id,
                'expires_in': signin_response.expires_in,
                'timestamp': int(time.time()),
                'auth_method': 'email_password',
                'email': env_email,
            }
            self._save_tokens()
            return signin_response.id_token
